...
uint8_t min = 0;
uint8_t hour = 0;
//uint8_t transmit[8] = "Navodaya";
uint8_t rx_buffer[5];

uint8_t main_cmd[PAYLOAD_LENGTH];
int i = 0;
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP */
void DioIrqHandlr(RadioIrqMasks_t radioIrq);
...
int main(void) {
	...
	pkt_params.Params.Gfsk.PreambleLength = 8;/*Covert byte into bit*/
	...
	SUBGRF_SetTxParams(RFO_LP, POWER, RAMP_TIME); //Set to RFO _LP for low power
